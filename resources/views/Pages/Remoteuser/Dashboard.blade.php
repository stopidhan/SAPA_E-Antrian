...
<body class="bg-gray-200 antialiased">
    ...
    @php
        // Logika perhitungan dan warna telah dipindahkan ke Controller untuk kode yang lebih bersih.
    @endphp
        ...
        @if (!$hasActiveQueue)
            <div class="mx-5 mt-4">
                <div class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-3.5">
                    ...
                    <p class="text-[13px] text-emerald-800 leading-snug"><strong>Selamat Datang,
                            {{ $namaUser }}!</strong> Akun Anda berhasil terdaftar.</p>
                </div>
            </div>
        @endif
        ...
        <div class="flex-1 px-4 sm:px-5 pt-5 pb-8">
            ...
            @if ($hasActiveQueue)
                <div class="mb-5 space-y-3">
                    ...
                    {{-- Card Antrean Aktif --}}
                    <div
                        class="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl p-5 shadow-lg shadow-blue-200/50">
                        <div class="flex items-center justify-between mb-2">
                            <p class="text-blue-200 text-[10px] font-semibold uppercase tracking-widest">Antrean Aktif
                            </p>
                            <span
                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-white/20 text-white text-[10px] font-semibold rounded-full">
                        ...
                        <p class="text-blue-200 text-xs mb-3">
                            {{ $activeQueue->service->service_name ?? 'Layanan Tidak Diketahui' }} &middot; est. ± 20
                            menit</p>

                        {{-- Countdown Timer --}}
                        <div class="bg-white/15 rounded-xl px-3 py-2.5 mb-3" x-data="countdown('{{ now()->addMinutes(22)->toIso8601String() }}')"
                            x-init="startTimer()">
                            <p class="text-[10px] text-blue-200 font-semibold text-center mb-1.5" x-show="!expired">
                                Segera menuju instansi dalam</p>
                            <p class="text-[10px] text-red-300 font-bold text-center mb-1.5" x-show="expired" x-cloak>
                                Waktu kedatangan habis!</p>
                            <div class="flex items-center justify-center gap-1.5" x-show="!expired">
                                ...
                                <span class="text-white/50 font-black text-sm">:</span>
                                <div class="bg-white/20 rounded-lg px-2 py-1 min-w-[34px] text-center">
                                    <p class="text-sm font-black" :class="seconds <= 10 ? 'text-red-300' : 'text-white'"
                                        x-text="seconds">00</p>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div class="flex items-center gap-2 px-4 py-2.5 bg-emerald-50 border-b border-emerald-100">
                            ...
                            <p class="text-[11px] font-semibold text-emerald-700">Tiket Tersimpan — Siap Scan di Kiosk
                            </p>
                        </div>
                        <div class="p-4">
                            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                {{-- Mini QR Preview (Sangat Besar) --}}
                                <a href="{{ route('booking.tiket') }}" class="shrink-0">
                                    <div
                                        class="w-28 h-28 bg-white rounded-2xl border-2 border-gray-100 flex items-center justify-center hover:border-blue-300 transition overflow-hidden p-2 shadow-sm">
                            ...
                            <div class="mt-3 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2.5">
                                <p class="text-[10px] font-semibold text-blue-700 uppercase tracking-wider mb-1.5">Cara
                                    Check-in di Kiosk:</p>
                                <ol class="space-y-1">
                                    <li class="flex items-center gap-2 text-[11px] text-blue-600">
                                        <span
                                            class="w-4 h-4 bg-blue-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center shrink-0">1</span>
                                        Datang ke instansi tujuan
                                    </li>
                                    <li class="flex items-center gap-2 text-[11px] text-blue-600">
                                        <span
                                            class="w-4 h-4 bg-blue-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center shrink-0">2</span>
                                        Buka QR Code dari tiket tersimpan
                                    </li>
                                    <li class="flex items-center gap-2 text-[11px] text-blue-600">
                                        <span
                                            class="w-4 h-4 bg-blue-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center shrink-0">3</span>
                    ...
                    <div class="flex items-start gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                        ...
                        <p class="text-[11px] text-red-700 leading-relaxed">Anda memiliki antrean aktif.
                            <strong>Selesaikan dulu</strong> sebelum mengambil nomor baru.
                        </p>
                    </div>
                </div>
            @endif
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4">
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-3">
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-xs font-bold text-gray-900">Kuota Booking Online Hari Ini</p>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-2.5 sm:gap-3 mb-3">
                        <div class="bg-indigo-50 rounded-xl px-3 sm:px-4 py-3 text-center flex flex-col gap-1">
                            <p class="text-xl sm:text-xl font-black text-indigo-700 leading-tight">{{ $totalKuota }}
                            </p>
                        ...
                        <div class="bg-emerald-50 rounded-xl px-3 sm:px-4 py-3 text-center flex flex-col gap-1">
                            <p class="text-xl sm:text-xl font-black text-emerald-600 leading-tight">
                                {{ $sisaKuota }}</p>
                        ...
                        <div class="bg-gray-50 rounded-xl px-3 sm:px-4 py-3 text-center flex flex-col gap-1">
                            <p class="text-xl sm:text-xl font-black text-gray-700 leading-tight">{{ $totalTerisi }}
                            </p>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:gap-3">
                        ...
                        <p class="text-[10px] text-gray-500 mt-1 sm:mt-0 text-right sm:text-left sm:min-w-[110px]">
                            {{ $totalKuota > 0 ? round(($totalTerisi / $totalKuota) * 100) : 0 }}% kuota terisi</p>
                    </div>
                </div>
            @endif
            ...
            @if ($totalKuota > 0 && $sisaKuota <= 0 && !$hasActiveQueue)
                <div
                    class="mb-4 flex items-start gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3.5 shadow-sm">
                    <span class="shrink-0 text-base">⚠️</span>
                    <p class="text-xs text-red-700 leading-relaxed"><strong>Mohon Maaf.</strong> Kuota pendaftaran
                        online untuk hari ini telah habis. Silakan coba kembali besok atau ambil antrean langsung
                        (On-site) di lokasi instansi.</p>
                </div>
            @endif

            @php
                $isFull = $totalKuota > 0 && $sisaKuota <= 0;
                $isDisabled = $hasActiveQueue || $isFull;
            @endphp
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach ($layanans as $svc)
                    @php
                        // Ambil warna berdasarkan prefix (A, B, C)
                        $prefix = strtoupper($svc->queue_prefix);
                        $c = $colorMap[$prefix] ?? $defaultColor;

                        // Slug untuk URL (digunakan di proses pendaftaran)
                    ...
                    <div x-data="{ expanded: false }"
                        ...
                        <div class="flex items-center gap-3.5 p-4 pb-3">
                            <div
                                class="w-11 h-11 {{ $c['bg'] }} rounded-xl flex items-center justify-center shrink-0 shadow-sm">
                        ...
                        <div x-show="expanded" x-cloak x-transition:enter="transition ease-out duration-200"
                            ...
                            {{-- Deskripsi --}}
                            <div class="pt-3">
                                <p class="text-[11px] font-semibold text-gray-700 uppercase tracking-wider mb-1">
                                    Deskripsi</p>
                            ...
                            {{-- Persyaratan (Placeholder Dinamis) --}}
                            <div>
                                <p class="text-[11px] font-semibold text-gray-700 uppercase tracking-wider mb-2">
                                    Persyaratan Umum</p>
                                <ul class="space-y-1.5">
                                    <li class="flex items-start gap-2">
                                        <svg class="w-3.5 h-3.5 {{ $c['text'] }} shrink-0 mt-0.5" fill="none"
                                            stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M4.5 12.75l6 6 9-13.5" />
                                        </svg>
                                        <span class="text-xs text-gray-600 leading-relaxed">Warga Negara Indonesia
                                            (WNI)
                                        </span>
                                    </li>
                                    <li class="flex items-start gap-2">
                                        ...
                                        <span class="text-xs text-gray-600 leading-relaxed">Terdaftar di wilayah
                                            instansi terkait</span>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div>
                                <p class="text-[11px] font-semibold text-gray-700 uppercase tracking-wider mb-2">
                                    Dokumen yang Dibawa</p>
                                <ul class="space-y-1.5">
                                    <li class="flex items-start gap-2">
                                        <span class="text-xs shrink-0">📋</span>
                                        <span class="text-xs text-gray-600 leading-relaxed">Kartu Keluarga & KTP
                                            Asli</span>
                                    </li>
                                </ul>
                            </div>
                        ...
                        {{-- Tombol Pilih --}}
                        <div class="px-4 pb-4">
                            <a href="{{ $isDisabled ? '#' : route('booking.konfirmasi', ['layanan' => $slug]) }}"
                                class="flex items-center justify-center gap-1.5 w-full py-3 {{ $c['btnBg'] }} text-white text-xs font-bold rounded-xl shadow-sm transition {{ $isDisabled ? 'pointer-events-none cursor-not-allowed opacity-50' : '' }}">
                                Pilih Layanan Ini
            ...
            <div class="mt-5">
                <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-start gap-2">
                    ...
                    <p class="text-[11px] text-blue-700 leading-relaxed">Kuota pendaftaran online di-reset setiap pukul
