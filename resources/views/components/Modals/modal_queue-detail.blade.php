    ...
    <x-modal name="service-detail" :show="$show" maxWidth="md">
        <div class="p-6 space-y-4">
            ...
            <div class="border-b pb-4 flex justify-between items-start">
                ...
                <div class="bg-blue-100 text-blue-800 text-2xl font-black px-4 py-2 rounded-lg" x-text="selectedQueue?.queue_number || '-'"></div>
            </div>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-gray-50 p-3 rounded-lg border">
                        ...
                        <p class="font-semibold text-gray-900 capitalize" x-text="selectedQueue?.registration_type || '-'"></p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-gray-50 p-3 rounded-lg border">
                        ...
                        <p class="font-semibold text-gray-900" x-text="selectedQueue?.created_at ? new Date(selectedQueue.created_at).toLocaleString('id-ID') : '-'"></p>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-lg border">
                        <p class="text-xs text-gray-500 mb-1">Waktu Dilayani</p>
                        <p class="font-semibold text-gray-900" x-text="selectedQueue?.start_at ? new Date(selectedQueue.start_at).toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'}) : '-'"></p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="bg-gray-50 p-3 rounded-lg border">
                        ...
                        <p class="font-semibold text-gray-900" x-text="selectedQueue?.completed_at ? new Date(selectedQueue.completed_at).toLocaleTimeString('id-ID', {hour: '2-digit', minute:'2-digit'}) : '-'"></p>
                    </div>
                    <div class="bg-gray-50 p-3 rounded-lg border">
                        <p class="text-xs text-gray-500 mb-1">Durasi Pelayanan</p>
                        <p class="font-semibold text-gray-900" x-text="selectedQueue?.service_time ? selectedQueue.service_time + ' menit' : '-'"></p>
                    </div>
                </div>
                    ...
                    <div class="bg-gray-50 p-3 rounded-lg border">
                        ...
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize"
                            :class="{
                                ...
                                'bg-gray-100 text-gray-800': !['completed', 'waiting', 'serving'].includes(selectedQueue?.status)
                ...
                <!-- Foto Antrean -->
                <div x-show="selectedQueue?.photos && selectedQueue.photos.length > 0" class="bg-gray-50 p-3 rounded-lg border mt-4">
                    <p class="text-xs text-gray-500 mb-2">Foto Antrean</p>
                    <div class="flex flex-wrap gap-3">
                        <template x-for="(photo, index) in selectedQueue?.photos" :key="index">
                            <a :href="photo" target="_blank" class="block border rounded-lg overflow-hidden shadow-sm hover:opacity-90 transition-opacity bg-white">
